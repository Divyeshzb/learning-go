...
func Testexecute(t *testing.T) {
	...
	type testCase struct {
		...
		args        []string
		expectedErr error
		prepare     func() *cobra.Command // Return the command to be executed
	}
	...
	tests := []testCase{
		{
			...
			prepare: func() *cobra.Command {
				...
				simpleCmd.Run = func(cmd *cobra.Command, args []string) {}
				return simpleCmd
			},
		},
		{
		...
		},
		{
			...
			prepare: func() *cobra.Command {
				simpleCmd := &cobra.Command{Use: "simple"}
				rootCmd.AddCommand(simpleCmd)
				simpleCmd.Flags().StringP("name", "n", "", "Name of the entity")
				simpleCmd.MarkFlagRequired("name") // TODO: Modify if flag structure changes
				return simpleCmd
			},
		},
		{
		...
		},
		{
			...
			expectedErr: nil,
			prepare: func() *cobra.Command {
				envCmd := &cobra.Command{Use: "env-test"}
	...
	rootCmd := &cobra.Command{Use: "root"} // Define a common rootCmd

	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			rootCmd.ResetCommands() // Clear previous commands to avoid conflicts
			tcCmd := tc.prepare()
			...
			err := tcCmd.Execute() // Execute the specific command
